  ...
  return (
    ...
    >
      ...
      <div className={`flex-1 md:px-10 ${isLeft ? "md:text-right" : "md:text-left"}`}>
        <div className={`group inline-block rounded-2xl border border-border bg-white p-6 shadow-sm transition-all duration-300 hover:shadow-xl hover:shadow-sky/10 hover:-translate-y-1 text-left`}>
          <span className="text-xs font-bold uppercase tracking-wider text-sky">
            {milestone.date}
          </span>
          <h3 className="mt-1 text-base font-bold text-deep-blue font-display">
            {milestone.title}
          </h3>
...
export function TimelineSection() {
  ...
  return (
    <section id="journey" className="relative bg-white py-24 overflow-hidden">
      {/* Subtle radial glow in center */}
      <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
        <div className="h-[600px] w-[600px] rounded-full bg-light-blue/50 blur-3xl" />
      </div>
      ...
      <div className="relative mx-auto max-w-5xl px-6">
        ...
        >
          <span className="section-label text-sky">Our Journey</span>
          <h2
        ...
        {/* Student quote */}
        <div className="mt-20 relative overflow-hidden rounded-3xl border border-gold/20 bg-gradient-to-br from-gold/5 to-light-blue/30 px-8 py-12 text-center">
          <div className="absolute -top-6 -right-6 h-24 w-24 rounded-full bg-gold/10 blur-2xl" />
          <Quote className="mx-auto h-8 w-8 text-gold" />
          <blockquote
            className="mx-auto mt-5 max-w-2xl text-xl font-bold italic leading-relaxed text-deep-blue md:text-2xl font-display"
          >
