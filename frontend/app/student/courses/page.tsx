    ...
    <div className="p-6 lg:p-8 space-y-10 bg-[#050B14] min-h-screen text-foreground animate-in fade-in duration-500">
      ...
      <div className="space-y-6">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-card border border-border rounded-3xl backdrop-blur-xl max-w-xl mx-auto">
            <HelpCircle className="h-12 w-12 text-slate-600 mx-auto mb-4" />
            <p className="text-muted-foreground font-bold text-lg">Catalog Empty</p>
                  ...
                  <div>
                    <div className="flex justify-between items-start mb-4">
                      ...
                      {unlocked ? (
                        ...
                        <span className="flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-slate-500/10 text-muted-foreground border border-border text-[9px] font-black uppercase tracking-widest">
                    ...
                    <h3 className="text-xl font-bold text-foreground mb-2 group-hover:text-[#2FA8CC] transition-colors leading-tight">
                      {course.course_name}
                    </h3>
                  ...
                  <div className="space-y-4 pt-4 border-t border-border">
                    ...
                    <div className="space-y-2">
                      <button
                        ...
                        className="w-full h-9 rounded-xl bg-card hover:bg-white/10 text-muted-foreground text-[10px] font-bold uppercase tracking-widest border border-border transition-all flex items-center justify-center gap-1"
                      >
                        {isExpanded ? (
                          <>
                          ...
                          {(!course.subjects || course.subjects.length === 0) ? (
                            ...
                            course.subjects.map((sub: any) => {
                              ...
                              return (
                                <div 
                                  key={sub._id} 
                                  className="p-2.5 bg-white/[0.01] border border-border rounded-xl flex items-center justify-between gap-2"
                                >
                                  <div className="min-w-0">
                                    <span className="text-[11px] font-bold text-foreground block truncate">{sub.subject_name}</span>
                    ...
                    <div className="pt-2">
                      {unlocked ? (
                        <Link 
                          ...
                          className="w-full h-11 bg-card hover:bg-[#2FA8CC]/10 text-foreground hover:text-[#2FA8CC] text-xs font-bold uppercase tracking-widest rounded-xl border border-border transition-all flex items-center justify-center gap-1.5"
                        >
                          View Study Hub
                        </Link>
                      ) : (
